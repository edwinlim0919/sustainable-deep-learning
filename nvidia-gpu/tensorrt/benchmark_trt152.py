...
import numpy as np
import argparse
from torchvision.models import ResNet152_Weights  # Update the import for ResNet-152
from pathlib import Path


# loads pre-trained resnet152 model
def load_model():
    model = models.resnet152(weights=ResNet152_Weights.IMAGENET1K_V1).eval().to('cuda') 
    return model

# preprocesses image for resnet152
    ...
    with open(output_file, 'w') as f_out:
        ...
        with torch.no_grad():
            for i in range(1, nruns + 1):
                ...
                if i % 10 == 0:
                    print(f'Iteration {i}/{nruns}, ave batch time {np.mean(timings):.6f} ms')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Benchmark ResNet152 with TensorRT')  # Update description
    parser.add_argument(
        '--batch-size', 
        type=int, 
        default=1, 
